    ...
    def get_strip_menu_options(person):
        ...
        for item in person.outfit.get_unanchored():
            ...
            if new_willingness + (person.obedience-100) >= -20:
                ...
                display_string += "\n{color=#ff0000}{size=18}Costs: " + price_display + "{/size}{/color}"
                ...
                if taboo_break:
                    display_string = "{image=gui/extra_images/taboo_break_token.png} " + display_string + " {image=gui/extra_images/taboo_break_token.png}"
                pay_strip_list.append([display_string + "\n{color=#ff0000}{size=18}Too Slutty{/size}{/color} (disabled)", [item,-1]])
        ...
        return [pay_strip_list, other_options_list]
...
init 1 python:
    def get_strip_tease_menu_options(person, guy_state, girl_state):
        ...
        if person.has_role(hypno_orgasm_role) and not person.event_triggers_dict.get("hypno_orgasmed_recently", False):
            command_options.append(["Trigger an orgasm", "hypno_orgasm"])

        for next_position in girl_state.leads_to:
            if person.effective_sluttiness() < next_position[0].slut_requirement:
                command_options.append([next_position[1] + "\n{color=#ff0000}{size=18}Requires: " + get_red_heart(next_position[0].slut_requirement) + "{/size}{/color} (disabled)", next_position[0]])
            else:
        ...
        return [action_options, command_options, strip_options]
            ...
            if the_person.effective_sluttiness("touching_body") < girl_state.slut_requirement + 20:
                ...
                if for_pay:
                    ...
                    menu:
                        ...
                        "Order her\n{color=#ff0000}{size=18}Requires: 125 Obedience{/size}{/color} (disabled)" if the_person.obedience < 125:
                            pass
                        ...
                        menu:
                            "Pay her\n{color=#ff0000}{size=18}Costs: $[strip_cost]{/size}{/color}" if mc.business.has_funds(strip_cost) and for_pay:
                                ...
                                if girl_state.is_close and top_layer and top_layer.underwear: #Slip it into her g-string or similar.
                                    ...
                                    mc.name "You drive a hard bargain. Here you go..."
...
label strip_tease_remove(the_person, the_clothing, girl_state, girl_direction, guy_state, for_pay = False):
    ...
    $ resist_level = None # Just a flag so we can know how much she resists and have some matching dialogue.
...
label strip_tease_guy_cum(the_person, girl_state, girl_direction, guy_state, for_pay = False): #TODO: The rest of this
    ...
    if not guy_state == "jerking":
        $ climax_controller = ClimaxController(["Pull out your cock", None], ["Cum in pants", "air"])
        ...
        if the_choice == "Pull out your cock":
            ...
            $ already_came = True
            #TODO: Possibility that she notices here and judges you.
...
label strip_cum_question_loop(the_person, girl_state, girl_direction, girl_aware, for_pay):
    ...
    if the_choice == "Cum on the floor":
        ...
        if girl_aware or girl_direction == "towards":
            if the_person.effective_sluttiness() < 20:
                "[the_person.title] watches, speechless."
            ...
            "She glances back at you and immediately realizes what's happening."
            if the_person.effective_sluttiness() < 20:
