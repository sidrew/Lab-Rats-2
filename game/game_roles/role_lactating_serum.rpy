...
label milk_for_serum_label(the_person): #Note that thee serum types have already had the "Milk" component added to them and are unconnected copies of the origional.
        ...
        menu:
            "I just need a little bit...":
                ...
                if the_person.event_triggers_dict.get("serum_in_breasts", 0) < 1:
                    ...
                    $ mc.log_event("Received 1 dose of " + milk_serum.name, "float_text_blue")
                    ...
                    $ mc.log_event("Received 2 doses of " + milk_serum.name, "float_text_blue")
                    "She hands you the vial and quickly tries to change the subject."
    ...
    if milking_allowed:
        ...
        if the_person.outfit.can_half_off_to_tits(visible_enough = False):
            $ generalised_strip_description(the_person, the_person.outfit.get_half_off_to_tits_list(visible_enough = False), half_off_instead = True)
        else:
            $ generalised_strip_description(the_person, the_person.outfit.get_tit_strip_list(visible_enough = False))
            ...
            "Milk her yourself" if the_person.effective_sluttiness() >= sluttiness_required or the_person.has_role(girlfriend_role) or the_person.has_role(affair_role) or the_person.has_role(breeder_role):
                ...
                if available_doses == 0:
                    ...
                    if available_doses > 0:
                        ...
                        $ mc.log_event("Received " + str(available_doses) + " dose of " + milk_serum.name, "float_text_blue")

                    "Job complete, you put the bottle away."


            "Milk her yourself\n{color=#ff0000}{size=18}Requires: [sluttiness_token]{/size}{/color} (disabled)" if the_person.effective_sluttiness() < sluttiness_required:
                pass
