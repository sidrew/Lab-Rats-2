...
#TODO: write our requiprements. Remember to check if "currently_interrogating" is True if it's explicitly for a bribe, if not we've met her somewhere else!
init -1 python:
    def city_rep_negotiate_requirement(the_person):
        if attention_floor_increase_2_policy.is_owned():
            return False
        elif not the_person.event_triggers_dict.get("currently_interrogating", False):
            return False
        return True
